...
device: Windows
---
这组 15 分题按核心数据结构与处理方式分为五类：哈希映射、结构体、按题意模拟、字符串和矩阵。先判断题目属于哪一类，再检查输入边界、状态记录和输出格式。
...
> 常见实现中，`int` 是 32 位有符号整数，范围约为 $-2^{31}$ 到 $2^{31}-1$；`long long` 通常是 64 位，范围约为 $-2^{63}$ 到 $2^{63}-1$。1000 位整数远超这两种类型的范围，应按字符串处理。
...
本题有两个注意点：先用 `memset(s, 0, sizeof(s))` 初始化计数数组，并包含 `<cstring>`；遍历字符串时，`str[i]` 是字符，需要用 `str[i] - '0'` 转换为对应数字。
...
得到全班排名后，在尚未分组的学生中，将名次最靠前的学生与名次最靠后的异性学生配对。每行输出一组姓名，分组顺序按前者名次从高到低排列。
...
输入第一行给出整数 N。“犯二的程度”是数字 2 的数量与位数之比；负数将结果乘以 1.5，偶数再乘以 2。
...
逐行检查字符串。如果其中包含区分大小写的 `qiandao` 或 `easy`，就跳过这道题。
